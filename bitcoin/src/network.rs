...
//!
//! assert_eq!(&bytes[..], &[0x42, 0x31, 0x54, 0x46]);
//! ```
...
impl Network {
    ...
    ///
    /// assert_eq!(Ok(Network::Bitcoin), Network::try_from(Magic::from_bytes([0x42, 0x31, 0x54, 0x46])));
    ...
    ///
    /// let network = Network::Bitcoin;
    /// assert_eq!(network.magic(), Magic::from_bytes([0x42, 0x31, 0x54, 0x46]));
    /// ```
...
mod tests {
    ...
    #[test]
    fn serialize_test() {
        assert_eq!(serialize(&Network::Bitcoin.magic()), &[0x42, 0x31, 0x54, 0x46]);
        assert_eq!(serialize(&Network::Testnet.magic()), &[0x38, 0x74, 0x42, 0x31]);
        assert_eq!(serialize(&Network::Signet.magic()), &[0x38, 0x74, 0x42, 0x31]);
        assert_eq!(serialize(&Network::Regtest.magic()), &[0x52, 0x45, 0x47, 0x42]);

        assert_eq!(deserialize(&[0x42, 0x31, 0x54, 0x46]).ok(), Some(Network::Bitcoin.magic()));
        assert_eq!(deserialize(&[0x38, 0x74, 0x42, 0x31]).ok(), Some(Network::Testnet.magic()));
        assert_eq!(deserialize(&[0x38, 0x74, 0x42, 0x31]).ok(), Some(Network::Signet.magic()));
        assert_eq!(deserialize(&[0x52, 0x45, 0x47, 0x42]).ok(), Some(Network::Regtest.magic()));
    }
